...
always @(*) begin
    case (state)
        ...
        TRANSFER: begin
            ...
            reset_crc <= (count == 0);
            reset_crc_checker <= (count == 0);
            MOSI <= (count < Frame_Size) ? out_buf_P2S : crc_out[Frame_Size + 4 - count];
            SCLK <= (count <= Frame_Size + 4) ? REFCLK : 0;
...
localparam IDLE = 0, LOAD_DT = 1, LOAD_CS = 2, TRANSFER = 3;
reg [1:0] state = IDLE;
reg [1:0] next_state;
...
always @(posedge REFCLK) begin
    ...
    case (state)
        ...
        LOAD_CS: begin
            count <= 0;
            SS <= (buf_input < Number_Slave) ? {Number_Slave{1'b1}} & ~(1 << buf_input[$clog2(Number_Slave)-1:0]) : {Number_Slave{1'b1}};
        end
        TRANSFER: begin
...
always @(*) begin
    case (state)
        IDLE: begin
            ...
            MOSI <= out_buf_P2S;
            SCLK <= 0;
            next_state <= (CNTL == 1)? LOAD_DT : ((CNTL == 2)? LOAD_CS : ((CNTL == 3) ? TRANSFER : IDLE));
        end
        LOAD_DT: begin
            READY <= 1'b1;
            VALID <= 1'b1;
            M_load <= (CNTL == 1) | (CNTL == 2);
            M_ena <= 0;
            S_ena <= 0;
            reset_crc <= 1'b1;
            reset_crc_checker <= 1'b1;
            MOSI <= 1'b0;
            SCLK <= 0;
            next_state <= (CNTL == 1)? LOAD_DT : ((CNTL == 3)? TRANSFER : ((CNTL == 2) ? LOAD_CS: IDLE));
        end
        LOAD_CS: begin
            READY <= 1'b1;
            VALID <= 1'b1;
            ...
            MOSI <= 1'b1;
            SCLK <= 0;
            next_state <= (CNTL == 2)? LOAD_CS : ((CNTL == 1)? LOAD_DT : IDLE);
        end
        TRANSFER: begin
            READY <= 1'b0;
            VALID <= (count == Frame_Size + 4)? t_valid : 1'b1;
            M_load <= 0;
            M_ena <= 1;
            S_ena <= count < Frame_Size + 4;
            reset_crc <= (count == 0);
            reset_crc_checker <= (count == 0);
            MOSI <= (count < Frame_Size) ? out_buf_P2S : crc_out[Frame_Size + 4 - count];
            SCLK <= (count < Frame_Size + 4) ? REFCLK : 1'b0;
            next_state <= (count == Frame_Size + 4)? ((CNTL == 3)? TRANSFER : ((CNTL == 0) ? IDLE : (CNTL == 1) ? LOAD_DT : LOAD_CS)) : TRANSFER;
        end
        default: begin
            READY <= 1'b0;
            VALID <= 1'b0;
